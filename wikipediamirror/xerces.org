...
* The final solution
  ...
  =data/xml-parse.sh= which is utilised by the makefiles to remove all
  ...
  conflicting articles are aboit 10-15, and are different depending
...
* Covering up with spaces
  ...
  scope of this project to debug fix that if we have a choice.
