...
** Compile time
   ...
   - Change =innodb_flush_method={{{ref(innodb_flush_method)}}} to
     ...
     problem with flushing large mapped files with =fsync= is that
     =fsync= searches for dirty pages in mapped memory pages linearly
     making it slower and slower as the file gets larger.
   - Calibrate the
