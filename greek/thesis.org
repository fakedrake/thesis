...
**** Term
     ...
     Ο TermClassifier απλά αναθέτει την κατηγορία wikipedia-term. Η
     Wikipediabase διαπραγματεύεται μόνο μόνο με πληροφορίες σχετικές
     με τη wikipedia.
...
***** Category regexes
      ...
      Χρησιμοποιεί τις ακόλουθες συνηθισμένες εκφράσεις για να
      ταυτίσει τις κατηγορίες ενός άρθρου.
...
***** Category matches
      ...
      Για παράδειγμα η σελίδα του Leonardo DiCaprio έχει τις ακόλουθες
      κατηγορίες:
      ...
      Όπως είναι φανερό η λίστα με τις κατηγορίες είναι αθυαίρετη και όχι
      πλήρης. Πολλαπλές μέθοδοι έχουν χρησιμοποιηθεί για να διορθωθεί αυτό.
      ...
      - Hand-pick κοινές κατηγορίες για άρθρα προσώπων ορισμένα ξανά με
      διαφορετικά κριτήρια

*** Resolvers

    Οι Resolvers είναι επίσης μονήρεις (singletons) αλλά ο σκοπός τους είναι
    να βρούνε την αξία από την αναζητούμενη ιδιοκτησία (property)

    . Όλοι οι resolvers προέρχονται από το BaseResolver

    και πρέπει να εφαρμόζουν τις ακόλουθες μεθόδους:
    ...
    Οι implemented resolvers είναι οι ακόλουθοι:

    Error the minimum priority resolver, it will always resolve to an

    error.

    Infobox Resolve attributes found on infoboxes of a symbol.

    Ένα πρόσωπο resolve τα ακόλουθα ειδικά χαρακτηριστικά των συμβόλων
    αναφερόμενα σε ανθρώπους :
    ...
    Κεφάλαια resolve το περιεχόμενοτων κεφαλαίων σε ένα άρθρο.
    ...
    - short-article /A summary of the article. Typically the/
...
*** Lisp types
    ...
    Ο τύπος Lisp είναι περιτυλίγματα (wrappers ) για python αντικείμενα ή
    τιμές που παρουσιάζονται σε μορφή s-expression που το START μπορεί να
    κατανοήσει . Έχουν δημιουργηθεί είτε από το ανεπεξέργαστο ερώτημα και
    έχουν ξετυλιχθεί (unwrapped) ώστε να είναι χρήσιμα στον αγωγό (pipeline
    ) , ή από την απάντηση που δίνει η WikipediaBase και στη συνέχεια
    κωδικοποιούνται σε ένα string και αποστέλλονται μέσω telnet στο START.
